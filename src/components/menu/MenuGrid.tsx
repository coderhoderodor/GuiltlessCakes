      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {items.map((scheduleItem) => {
          ...
          return (
            <div
              key={scheduleItem.id}
              className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-neutral-100"
            >
              {/* Image */}
              <div
                ...
                {item.dietary_tags?.length > 0 && (
                  <div className="absolute top-2 left-2 flex gap-1">
                    {item.dietary_tags.map((tag) => (
                      <span
                        key={tag}
                        className="bg-white/90 backdrop-blur-sm text-pink-600 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1"
                      >
                        {dietaryIcons[tag]}
                        {tag}
                      </span>
              ...
              <div className="p-4">
                <h3
                  className="font-semibold text-neutral-800 mb-1 cursor-pointer hover:text-pink-600 transition-colors"
                  onClick={() => setSelectedItem(scheduleItem)}
                >
                  {translation?.name}
                </h3>
      ...
      {selectedItem && (
        ...
        >
          <div className="grid md:grid-cols-2 gap-6">
            {/* Image */}
            <div className="aspect-square bg-pink-50 rounded-lg relative">
